    ...
    private static int executeStartRoll(CommandContext<CommandSourceStack> context) {
        try {
            ...
            switch (lootType.toLowerCase()) {
                case "weapon":
                    choiceManager.startRoll(targetPlayer, GeneHunterLootInit.WEAPON_LOOT_TYPE.get());
                ...
                case "xene":
                    choiceManager.startRoll(targetPlayer, BiotechLootTypeInit.XENE_TRAIT_LOOT_TYPE.get());
                    context.getSource().sendSuccess(
                            () -> net.minecraft.network.chat.Component.translatable(
