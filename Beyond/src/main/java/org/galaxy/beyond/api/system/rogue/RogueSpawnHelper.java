...
public final class RogueSpawnHelper {
    ...
    public static Vec3 gatewayPos(ServerLevel level, BlockPos nodePos, int distance) {
        ...
        BlockPos aboveNode = base.above(2);
        if (isNodeBlock(level, base) && hasClearance(level, aboveNode, 3)) {
            return new Vec3(base.getX() + 0.5, aboveNode.getY(), base.getZ() + 0.5);
        }
    ...
    private static boolean hasClearance(ServerLevel level, BlockPos pos, int height) {
        for (int i = 0; i < height; i++) {
            if (!isEmpty(level, pos.above(i))) return false;
        }
        return true;
    }

    private static boolean isNodeBlock(ServerLevel level, BlockPos pos) {
        return level.getBlockState(pos).getBlock() instanceof NodeBlock;
    }
