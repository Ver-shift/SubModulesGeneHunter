...
public final class TerrainBlendJigsawStructure extends Structure {
    ...
    private static final int TERRAIN_SAMPLE_STEP = 16;
    ...
    protected Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        ...
        return JigsawPlacement.addPieces(
                ...
                this.liquidSettings
        ).flatMap(stub -> filterTerrain(context, stub));
    }

    private Optional<GenerationStub> filterTerrain(GenerationContext context, GenerationStub stub) {
        StructurePiecesBuilder builder = stub.getPiecesBuilder();
        if (builder.isEmpty() || hasSharpTerrainDrop(context, builder.getBoundingBox())) {
            return Optional.empty();
        }
        return Optional.of(stub);
    }

    private boolean hasSharpTerrainDrop(GenerationContext context, BoundingBox box) {
        Heightmap.Types heightmap = this.projectStartToHeightmap.orElse(Heightmap.Types.WORLD_SURFACE_WG);
        int radius = Math.max(TERRAIN_SAMPLE_STEP, this.terrainBlendConfig.blendRadius());
        int allowedDelta = this.terrainBlendConfig.maxAllowedSurfaceDrop();
        int minHeight = Integer.MAX_VALUE;
        int maxHeight = Integer.MIN_VALUE;

        for (int x = box.minX() - radius; x <= box.maxX() + radius; x += TERRAIN_SAMPLE_STEP) {
            for (int z = box.minZ() - radius; z <= box.maxZ() + radius; z += TERRAIN_SAMPLE_STEP) {
                ...
                if (maxHeight - minHeight > allowedDelta) {
                    return true;
                }
            }
        }
        int[] xSamples = {box.minX() - radius, box.maxX() + radius};
        int[] zSamples = {box.minZ() - radius, box.maxZ() + radius};
        for (int x : xSamples) {
            for (int z : zSamples) {
                int height = context.chunkGenerator().getFirstOccupiedHeight(
                        x,
    ...
    public BoundingBox adjustBoundingBox(BoundingBox boundingBox) {
        return super.adjustBoundingBox(boundingBox).inflatedBy(this.terrainBlendConfig.blendRadius() + 1);
    }
