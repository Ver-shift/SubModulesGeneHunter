...
## 项目结构
事件处理系统主要位于 Beyond 模组的 com.pz.beyond.event 包及其子包中，并与网络层、数据层、附件系统、安全区规则等模块协同工作。
...
## 架构总览
事件系统围绕 NeoForge 事件总线与自定义网络协议展开，采用“规则驱动 + 事件广播 + 网络同步”的架构模式。
...
Server->>Handlers : 触发 PlayerLoggedInEvent
Handlers->>Net : 发送 SafeZonePayload
...
## 依赖分析
- 事件处理器依赖于数据层（SafeZoneData、PlayerStateData）、网络层（SafeZonePayload、SafeZonePayloadUtil）、附件系统（ModAttachments）与规则系统（ISafeZoneRuleListener、SafeZoneRuleUtil）。
...
- 事件发布：PlayerStateChangeEvent 仅在状态切换时发布，避免频繁触发。
...
## 结论
本事件处理系统通过“规则 + 事件 + 网络”的组合，实现了安全区功能的高扩展性与高性能。SafeZoneHandler 与 PlayerStateHandler 负责核心业务逻辑，ModPayloadsHandler 提供可靠的网络同步，PlayerStateChangeEvent 为外部模块提供了灵活的接入点。借助 SafeZoneRuleUtil 的自动发现机制，第三方可轻松扩展新的安全区规则，满足多样化玩法需求。
