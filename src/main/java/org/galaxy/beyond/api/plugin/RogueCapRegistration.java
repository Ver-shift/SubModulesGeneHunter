...
public class RogueCapRegistration {
    ...
    public void addCap(Identifier id, Supplier<? extends RogueCap> supplier) {
        if (id == null || supplier == null) return;
        BeyondRogueCapInit.registerCap(id, checkedSupplier(id, supplier));
    }

    public void addCap(Identifier id, RogueCap cap) {
        if (cap == null) return;
        addCap(id, () -> cap);
    }

    public void addCap(Identifier id, Class<? extends RogueCap> capClass) {
        if (id == null || capClass == null) return;
        addCap(id, () -> newInstance(capClass));
    }

    private Supplier<? extends RogueCap> checkedSupplier(Identifier id, Supplier<? extends RogueCap> supplier) {
        return () -> {
