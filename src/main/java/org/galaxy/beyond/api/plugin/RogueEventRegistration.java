...
public class RogueEventRegistration {
    ...
    public void addEvent(Identifier id, Supplier<? extends RogueEventType> supplier) {
        if (id == null || supplier == null) return;
        BeyondEventInit.register(id, checkedSupplier(id, supplier)::get);
    }

    public void addEvent(Identifier id, RogueEventType event) {
        if (event == null) return;
        addEvent(id, () -> event);
    }

    public void addEvent(Identifier id, Class<? extends RogueEventType> eventClass) {
        if (id == null || eventClass == null) return;
        addEvent(id, () -> newInstance(eventClass));
    }

    private Supplier<? extends RogueEventType> checkedSupplier(Identifier id, Supplier<? extends RogueEventType> supplier) {
        return () -> {
